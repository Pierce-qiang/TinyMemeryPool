...
void testBasicAllocation() 
{
    ...
    // 测试小内存分配
    void* ptr1 = MemoryPool::alloc(8);
    assert(ptr1 != nullptr);
    MemoryPool::dealloc(ptr1, 8);

    // 测试中等大小内存分配
    void* ptr2 = MemoryPool::alloc(1024);
    assert(ptr2 != nullptr);
    MemoryPool::dealloc(ptr2, 1024);

    // 测试大内存分配（超过MAX_BYTES）
    void* ptr3 = MemoryPool::alloc(1024 * 1024);
    assert(ptr3 != nullptr);
    MemoryPool::dealloc(ptr3, 1024 * 1024);
...
void testMemoryWriting() 
{
    ...
    char* ptr = static_cast<char*>(MemoryPool::alloc(size));
    assert(ptr != nullptr);
        ...
        try 
        {
            ...
            for (int i = 0; i < ALLOCS_PER_THREAD && !has_error; ++i) 
            {
                ...
                if (rand() % 2 && !allocations.empty()) 
                {
                    ...
                    MemoryPool::dealloc(allocations[index].first, 
            ...
            {
                MemoryPool::dealloc(alloc.first, alloc.second);
            }
        }
...
void testEdgeCases() 
{
    ...
    // 测试0大小分配
    void* ptr1 = MemoryPool::alloc(0);
    assert(ptr1 != nullptr);
    MemoryPool::dealloc(ptr1, 0);
    
    // 测试最小对齐大小
    void* ptr2 = MemoryPool::alloc(1);
    assert(ptr2 != nullptr);
    assert((reinterpret_cast<uintptr_t>(ptr2) & (ALIGNMENT - 1)) == 0);
    MemoryPool::dealloc(ptr2, 1);
    
    // 测试最大大小边界
    void* ptr3 = MemoryPool::alloc(MAX_BYTES);
    assert(ptr3 != nullptr);
    MemoryPool::dealloc(ptr3, MAX_BYTES);
    
    // 测试超过最大大小
    void* ptr4 = MemoryPool::alloc(MAX_BYTES + 1);
    assert(ptr4 != nullptr);
    MemoryPool::dealloc(ptr4, MAX_BYTES + 1);
