...
public:
    ...
    static void warmup() 
    {
        ...
        {
            MemoryPool::dealloc(ptr, size);  // 使用实际分配的大小进行释放
        }
    ...
    static void testSmallAllocation() 
    {
        ...
        // 测试内存池
        {
            ...
            for (void* ptr : ptrs) 
            {
                MemoryPool::dealloc(ptr, SMALL_SIZE);
            }
    ...
    static void testMultiThreaded() 
    {
        ...
        auto threadFunc = [](bool useMemPool) 
        {
            ...
            {
                size_t size = dis(gen);
                void* ptr = useMemPool ? MemoryPool::alloc(size) 
                                     : new char[size];
                ...
                {  // 75%的概率释放
                    ...
                    if (useMemPool) {
                        MemoryPool::dealloc(ptrs[index].first, ptrs[index].second);
                    } else {
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) 
            {
                ...
                if (i % 100 == 0 && !ptrs.empty()) 
                {
                    ...
                    {
                        MemoryPool::dealloc(ptrs.back().first, ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
            }
...
int main() 
{
    ...
    MemoryPool::destroy();  // 销毁内存池

    // void* mem = mi_malloc(100); // 分配100字节的内存
    // // ...使用mem...
    // mi_free(mem); // 释放内存
    return 0;
}